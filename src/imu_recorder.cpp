...
IMU_Recorder::IMU_Recorder(string filename_) : time_to_exit(false), filename(filename_) {
    unEmptyIMU = PTHREAD_COND_INITIALIZER;
    emptyIMU = PTHREAD_COND_INITIALIZER;
    mutexIMU = PTHREAD_MUTEX_INITIALIZER;

    std::cout << "initialize imu recorder for " << filename << std::endl;
}
...
void IMU_Recorder::write_imu_from_queue() {
    ...
    datasetimu.open("./record_data/" + filename);
